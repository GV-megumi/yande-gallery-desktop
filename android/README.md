...
1. **桌面端开启「设置 → API 服务 → 手机端连接 → 允许手机端连接」**
   这是安卓端全部接口——服务信息/健康检查、同步 5 路由、图集与图片写 9 路由、缩略图/预览/原图
   3 档二进制、SSE 事件——共同挂载的手机面 `/api/app/v1/*` 唯一的门：开启即自动运行服务器并强制
...
2. **⚠️ 该开关默认关闭；不开，手机端一切请求一律 403**
   ...
   > 但配对后的时间轴/相册元数据同步、缩略图与原图加载、删图/标签编辑/建改删图集等写操作、SSE
   > 事件订阅——全部请求都会返回 `403`。**
   >
...
- [ ] **写操作端到端**：开启「允许手机端连接」后，删图 / 批量删 / 标签编辑 / 建改删图集 /
