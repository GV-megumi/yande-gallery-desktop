...
- **桌面域「复制到」（F6）**：多选栏与大图页的「加入相册」入口改名「复制到」，选择器升级为两节
  ...
  回滚 + 事后对账）+ 手机相册节（仅 29+ 且在线时渲染，离线时「复制到」入口本身整体置灰：对每张图
  `ImageMirrorStore.ensure(ORIGINAL)` 取原图 → 字节拷贝 `MediaStore.insert` 落目标相册，WorkManager
  后台串行 + 计数进度通知「正在复制到手机相册 x/y」+ 404 跳过 / 磁盘满中止的镜像同款分流，
  完成后有失败张则弹汇总通知「成功 x 张，y 张失败」）。**成功 / 失败 snackbar 文案随
...
验证：全量 Robolectric 92 类 / 564 例 / 0 失败 / 0 错误（`gradlew :app:testDebugUnitTest` BUILD SUCCESSFUL，
含下节 v0.8.1 加固轮补强后的当前全量）；桌面 `npm run test`（typecheck + vitest）全绿。

### v0.8.1 手机相册加固轮（DEFER backlog 七类修复）

对齐 spec `doc/superpowers/specs/2026-07-21-device-albums-hardening-design.md`：v0.8.0 手机相册与复制/移动
体系合并后，在独立分支 `feature/device-albums-hardening` 上按 v0.8.0 终审 DEFER backlog（A–G）逐类清欠——
纯结构收敛 / 行为修复 / 健壮性 / 测试补强，**无新功能面、桌面端零改动**。versionCode 10 / versionName `0.8.1`。

- **A 复用收敛（零行为差纯重构）**：`findActivity` + 系统栏辅助（`applySystemBars`/`setSystemBarAppearanceLight`）
  三件套上收 `ui/common/SystemBarUtil.kt`（原三份/两份副本删除）；`Pictures/<名>/` 路径构造六点归一到
  `DeviceModels.pendingAlbumPath`；`DeviceMedia.mime()` 迁入 `DeviceModels`；DeviceAlbumPicker 与
  CopyTargetPicker 手机节 ~80 行镜像抽为共享 `DeviceAlbumSection`；可写过滤 `isWritableAlbumPath` 前置进
  `buildWritableTargets`，三个 VM（DeviceAlbumDetail/DeviceViewer/DeviceCopyTargets）目标候选与重名校验
  同层取快照，消除「与不可写 bucket 同名的新建跨页判定漂移」。
- **B 批量复制 WorkManager 化**：手机→手机多选复制由组合作用域改为 `DeviceCopyWorker`（对齐桌面→手机
  导出 worker 形态），离屏 / 杀进程可续跑，`findCopy` 查重防重复落盘，磁盘满中止 / 源删继续 / failed>0
  弹汇总通知；**大图页单张复制仍保持同步执行**（即时反馈优先）。
- **C RetryableAsyncImage 手势让位**：三选择网格（Photos / AlbumDetail / Device）失败格在多选态下点击 /
  长按透传给格子可选中，重试入口下沉为 28dp 角标按钮；非多选态保留整格点击重试、长按恒进多选。
- **D 导出防御**：`enqueue` 返回成败，serverId null / 入队异常时 UI 提示「复制启动失败」而非假成功；
  `findCopy` 的 `resolver.query` 包 runCatching，OEM 异常降级为查无副本、放行 insert。
- **E 移动进程重建诚实降级**：网格页移动授权回调若选中集因进程重建为空 → 静默放弃（不调 moveTo、
  不弹 snackbar），消除「已移动 0 张」误导。
- **F 测试补强（全量清欠，本轮净增主力）**：按各任务 review Minor 清单逐条补齐——权限 accessLevel /
  BucketKey decode / moveTo 对账 / 待落地收编 / 分享 intent 组装 / 日期时间标签 / EXPORT_BATCH 分块 /
  call-log 顺序断言等锁行为用例。
- **G 小 UX**：picker 打开加载态（不闪旧快照）、多选底栏与 picker 行连点防抖（300ms）、汇总通知 id 按
  serverId 加盐防批间互覆、DeviceViewer settle 快照瞬空跳过清理（消除放大态外部脉冲回位）、桌面域
  Viewer 相册上下文移空返回、联调计划补退出标准行。

验证：全量 Robolectric **92 类 / 564 例 / 0 失败 / 0 错误**（`gradlew :app:testDebugUnitTest` BUILD
SUCCESSFUL；v0.8.0 基线 530 → 564，净增 34，F 类补强为主 + C/D/G/B 新增用例）；桌面 `npm run test`
（typecheck + vitest）全绿（本轮桌面零改动，`git diff master..HEAD -- src/` 为空）。手机→手机批量复制
离屏续跑真机冒烟无头环境不可驱动，登记 `docs/superpowers/plans/2026-07-05-M3实机联调计划.md` §L L.9 待人工。
