...
桌面端（yande-gallery-desktop）已管理大量本地图片（相册、标签、SQLite 元数据）。目标是做一个**安卓端相册 app**，通过**局域网 HTTP** 连接桌面端：
...
2. 安卓端：照片时间轴、相册（相册）、大图浏览、搜索、多选、下载原图到系统相册、设置（服务器/缓存管理）。
3. 安卓端：写操作——删除图片、相册管理（新建/重命名/删除、移入移出）、标签编辑，行为与桌面端一致。
...
| D9 | v1 包含全套写操作 | 删除图片、相册管理（新建/重命名/删除、移入移出）、标签编辑；用户明确要求进 v1（推翻早期只读设定） |
...
| `GET /api/v1/sync/galleries` | 相册全量（小表）：`{ id, name, coverImageId, imageCount }` |
...
- 权限均为 `galleryRead`。
- **配套约束（桌面端）**：凡影响某图片可同步元数据的变更——含标签增删、相册归属变化——必须触碰对应 `images.updatedAt`，否则增量同步会漏掉该变化。实现时审计现有写路径并补齐。
- 不下发 `filepath`（手机端不需要绝对路径，减小载荷与信息暴露）。

### 5.4 写操作接口（D9/D10）

全部写接口**复用桌面端既有 service 层**（`imageService` / 相册服务），不得绕过直写 SQL——确保删除/变更走同一套业务语义（booru 帖子复位、imageCount 回写、领域事件发布），并按 §5.3 的约束触碰相关 `images.updatedAt`。
...
| `POST /api/v1/galleries` | galleryWrite | `{ name }` 新建相册 |
| `PATCH /api/v1/galleries/:galleryId` | galleryWrite | `{ name }` 重命名相册 |
| `DELETE /api/v1/galleries/:galleryId` | galleryWrite | 删除相册（仅相册与归属关系，不删图片文件，与桌面端语义一致） |
| `POST /api/v1/galleries/:galleryId/images` | galleryWrite | `{ imageIds[] }` 图片移入相册 |
| `DELETE /api/v1/galleries/:galleryId/images` | galleryWrite | `{ imageIds[] }` 图片移出相册 |
...
- 相册网格卡片：封面（coverImageId，缺省取相册内最新图）+ 名称 + 张数。
- 点入为该相册网格页（与时间轴同构，无日期分组，按 createdAt 倒序）。
- 右上角「+」新建相册；长按相册卡片弹出菜单：重命名 / 删除（二次确认，说明不删图片文件）。

### 7.3 大图页

- 进入/返回使用共享元素转场；左右滑切换（Pager）。
- 手势：双击缩放循环（适配↔2x）、双指自由缩放（上限约 5x，1600 档像素不足时提示可查看原图）、单击切换沉浸模式（隐藏系统栏与操作栏）、**下滑拖拽关闭**返回网格。
- 底部操作栏：**分享**（未下载则先执行下载原图再唤起系统分享）／**查看原图**（=下载，D5；已下载显示「已保存」并直读本地）／**删除**（二次确认，D10）／**详情**／**更多**（加入相册、移出当前相册）。
- 详情面板（上滑或点详情）：文件名、分辨率、大小、格式、入库时间、标签（可点击跳搜索，**可编辑**：添加/移除，D9）、所属相册（可点击跳相册）。
...
- 网格长按进入多选：角标勾选、顶部计数 + 全选、底部操作栏 **下载 / 分享 / 删除 / 加入相册 / 取消**（在相册内浏览时另有「移出相册」）；批量下载进 WorkManager 队列，批量删除二次确认（明示数量）。
...
- **离线时写操作**：相关按钮置灰（删除/相册操作/标签编辑），不排队。
...
- **桌面端**（vitest，现有测试体系）：Range/206/416、ETag/304、Content-Type 映射；sync 游标分页正确性（边界：同 updatedAt 多行、空库）；image-ids 与 dataVersion 行为；preview 档生成与缓存复用；写接口全套（删除含 404 幂等与磁盘清理、相册增删改与成员变更、标签增删、`updatedAt` 触碰、领域事件发布、权限键校验）。
...
3. **M3 核心体验**：大图页（手势/转场/三档加载）、原图下载（MediaStore + WorkManager）、搜索、多选、写操作 UI（删除/相册管理/标签编辑）。
