# 图集与文件夹解耦 — 阶段 1（数据层 / Expand）实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在不破坏现有功能的前提下，引入 `gallery_folders` 与 `gallery_images` 两张关联表，并把现有图集的"文件夹绑定"和"图片归属"无损回填进去。
...
**本阶段对应 spec：** [2026-06-28-图集与文件夹解耦重构-design.md](../specs/2026-06-28-图集与文件夹解耦重构-design.md) 的 §4.2、§4.3、§11.1（步骤 1/3/4 的"扩张"部分）。后续阶段（服务原语、booru/app:// 关联、跨机器重定位、IPC/类型、渲染层、contract 重建）各自单独成计划。
...
## Task 2：回填 `gallery_folders`（每个旧图集一条文件夹绑定）
...
在测试文件末尾追加（核心正确性用例：递归图集含嵌套、非递归图集仅含直接图片）：
...
- [ ] **Step 3: 运行解耦迁移测试 + 既有图集相关测试，确认无回归**
...
Expected: PASS（新文件全绿；既有数据库/图集测试不受影响）。
...
git commit -m "feat: initDatabase 接入图集解耦迁移（Expand 阶段完成）"
```
...
**计划完成，保存在 `docs/superpowers/plans/2026-06-28-图集与文件夹解耦-阶段1-数据层.md`。两种执行方式：**
