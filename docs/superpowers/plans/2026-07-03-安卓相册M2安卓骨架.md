...
- Produces（spec §6.2）:
  ...
  - DAO 关键签名：
    ...
    - `GalleryDao.observeAll(): Flow<List<GalleryEntity>>`、`replaceAll(items)`（@Transaction 清表重插）、`coverFallback(galleryId): ImageEntity?`（相册内 createdAt 最新一张）、`galleryImagesPagingSource(galleryId): PagingSource<Int, ImageEntity>`
...
`GalleryDaoTest.kt`（replaceAll 覆盖旧数据、coverFallback 取相册内最新、galleryImagesPagingSource 按 createdAt 倒序）与 `ServerDaoTest.kt`（activate 事务后仅一行 isActive、observeActive 发射切换）同风格编写——用例断言照上面模式，覆盖 Interfaces 列出的每个 DAO 方法。
...
interface GalleryDao {
    ...
    /** 封面兜底：coverImageId 缺省时取相册内最新一张（spec §7.2）。 */
    @Query(
...
- [ ] **Step 1: 写失败测试** — `AlbumsViewModelTest.kt`（in-memory db 种子两个相册，其一 coverImageId=null 但有成员图）：`albums` 首发射两卡片；null 封面的卡片 coverImageId 取相册内最新图 id。
...
- [ ] **Step 5: Commit** — `git commit -m "feat(android): 相册 tab——相册卡片网格（封面兜底）与相册详情页"`
