...
1. **BooruPage** - 主页面
   - 图片列表（动态网格布局）
     - 按 ID 排序显示
     - 每行显示多张图片，每行高度取该行图片的最大高度
     - 响应式布局，根据窗口大小自动调整每行图片数量
     - 确保图片完整显示，不裁剪
   - 搜索栏（支持标签自动补全）
   - 站点选择器
   - 分级筛选器
...
3. **BooruPostDetailsPage** - 图片详情页面（已实现）
   - 全屏模态框显示
   - 大图预览（支持缩放、拖拽）
   - 完整信息展示（角色、艺术家、版权、创建时间、来源）
   - 标签列表（按分类显示，不同颜色）
   - 文件详情（ID、评分、大小、分辨率、格式、MD5、上传者）
   - 相关帖子
   - 收藏/下载操作
   - 键盘导航（左右方向键切换图片，ESC 关闭）
   - 原图缓存和预加载（前后3张）

4. **BooruDownloadPage** - 下载管理页面（已实现 ✅）
   - 下载队列展示
   - 实时进度显示
   - 暂停/恢复/取消操作
   - 按状态筛选（进行中/已完成/失败）

5. **BooruBulkDownloadPage** - 批量下载页面（已实现 ✅）
   - 显示活跃的批量下载会话
   - 创建新的批量下载任务
   - 管理批量下载会话（启动、暂停、取消、删除）

6. **BooruSettingsPage** - 站点设置页面（已实现 ✅）
   - 站点管理（添加、编辑、删除、激活）
   - 外观配置（网格大小、预览质量、分页设置等）
   - 文件名模板配置（支持多种 token）
   - 缓存管理（查看缓存大小、配置最大缓存大小）
...
#### 布局优化 (2025-11-19)
1. ✅ 实现动态网格布局（BooruGridLayout组件）
   - 按 ID 排序显示图片
   - 每行显示多张图片，每行高度取该行图片的最大高度
   - 响应式布局，根据窗口大小自动调整每行图片数量
   - 确保图片完整显示，不裁剪
2. ✅ 标签分类和颜色显示
   - 使用 tag/summary.json API 批量获取标签分类
   - 按分类显示不同颜色（artist=红色，character=绿色，copyright=紫色，meta=橙色，general=蓝色）
3. ✅ 图片详情页预加载
   - 打开详情页时，自动预加载前后3张图片的原图到缓存
4. ✅ 原图缓存功能
   - 支持将原图缓存到本地（data/cache目录）
   - 可配置缓存目录最大大小，自动清理旧缓存

### 10.2 短期目标 (待实现)

#### 标签搜索页面
- [ ] **详情页标签点击逻辑改进**：当前点击标签会把搜索加到主页，需要改成创建新的标签搜索页面
  - 点击标签后，打开新的标签搜索页面
  - 在该页面中搜索并加载对应标签的图片
  - 支持查看详情（复用 BooruPostDetailsPage）
  - 支持分页浏览
  - 支持返回上一页

#### 文件名生成器（已实现 ✅）
- [x] 创建filenameGenerator.ts
- [x] 实现模板解析
- [x] 支持标记替换（支持多种 token，如 {id}, {md5}, {extension}, {width}, {height}, {rating}, {score}, {site}, {artist}, {character}, {copyright}, {date}, {tags}, {source}）
- [x] 处理非法字符
- [x] 支持 token 选项（如 {md5:maxlength=8}）

#### 下载管理器（已实现 ✅）
- [x] 实现下载队列管理（downloadManager.ts）
- [x] 并发控制（最大3个，可配置）
- [x] 断点续传支持（支持取消和重试）
- [x] 进度回调（实时更新下载进度）
- [x] 智能去重（基于 MD5 检测重复图片）
- [x] 自动重试机制

### 10.3 中期目标

#### 高级页面
- [x] 创建BooruDownloadPage（下载管理页面）✅
- [x] 创建BooruSettingsPage（站点设置页面）✅
- [x] 创建BooruBulkDownloadPage（批量下载页面）✅
- [ ] 创建BooruFavoritesPage（收藏页面）

#### 功能增强
- [x] 批量下载 ✅（BooruBulkDownloadPage 已实现）
- [x] 下载队列管理 ✅（BooruDownloadPage 已实现）
- [ ] 热门图片展示（API 已实现，UI 待集成）
- [ ] 标签自动补全（API 通道已定义，处理器待实现）
- [ ] 搜索历史（API 通道已定义，处理器待实现）
- [ ] 收藏夹同步（部分实现，需要完善同步逻辑）

### 10.4 长期目标

#### 测试和优化
- [x] 性能优化（动态网格布局，图片懒加载，原图缓存）✅
- [ ] 完整功能测试（Yande.re API）
- [ ] 虚拟滚动（当前使用分页，可考虑优化）
- [ ] Bug修复
- [ ] 用户文档编写
...
#### 已实现的核心功能总结
- ✅ 多站点支持（Moebooru API 对接）
- ✅ 图片浏览（动态网格布局，按 ID 排序）
- ✅ 图片搜索（标签搜索，分级筛选）
- ✅ 图片详情页（全屏预览，缩放拖拽，键盘导航）
- ✅ 标签分类显示（按类别显示不同颜色）
- ✅ 原图缓存（自动缓存，可配置大小限制）
- ✅ 图片预加载（详情页前后3张预加载）
- ✅ 下载管理（队列管理，并发控制，进度显示）
...
**最后更新**: 2025-11-19
**作者**: Claude AI
**状态**: ✅ 核心功能已实现 - Booru功能基础版已完成，包括下载管理、批量下载、站点设置等主要功能
