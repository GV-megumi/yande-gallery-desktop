...
**Architecture:** 以“统一规则、统一入口、局部重构”为原则推进：下载相关问题统一收敛到批量任务服务层去重；Booru 左侧导航只保留一级+二级菜单，标签管理与下载通过右侧页内子导航承载原“三级”；图库页把“刷新当前展示”和“同步文件夹”拆成两个明确动作，并补齐相册编辑入口与统一 tooltip / 关闭交互规范。
...
- `tests/main/services/gallerySync.test.ts` 或追加到 `tests/main/services/galleryService.test.ts` — 当前相册目录同步逻辑测试。
...
- `src/main/services/galleryService.ts` — 当前相册目录同步、相册统计更新复用。
- `src/main/services/imageService.ts` — 复用目录扫描导入，不重写底层扫描。
- `src/renderer/pages/GalleryPage.tsx` — 同步按钮、刷新按钮 tooltip、相册右键编辑、编辑弹窗、详情布局调整。
...
### 复用现有能力
- `galleryService.updateGallery(...)` 已支持相册名称修改，可直接复用到编辑弹窗保存。
- `imageService.scanAndImportFolder(...)` 可复用为“当前相册目录同步”的底层实现。
...
git commit -m "feat: 新增相册级同步文件夹能力"
```

---

## Task 6: 图库页增加同步按钮、右键编辑与相册改名
...
- Modify: `src/renderer/pages/GalleryPage.tsx:944-980`（相册卡片区域）
...
it('shows separate refresh and sync actions for selected gallery', async () => {
  ...
  expect(screen.getByRole('button', { name: /刷新当前相册/i })).toBeInTheDocument()
...
const handleSaveGalleryEdit = async (values: { name: string }) => {
  ...
  if (result.success) {
    ...
    message.error(result.error || '相册更新失败')
  }
}
```

- [ ] **Step 5: 将相册卡片的更多操作入口放入右键菜单**
...
git commit -m "feat: 图库相册页新增同步文件夹与编辑入口"
```
