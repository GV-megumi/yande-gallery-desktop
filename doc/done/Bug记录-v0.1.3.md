...
- 预期：
  ...
  - 该固定下载目录应同时出现在本地相册列表中；如果目录对应相册不存在，应在创建目录后自动添加“收藏相册”。
...
- 待明确设计点：
  ...
  - 固定目录创建后应检查 `galleries.folderPath` 是否已有对应相册；没有则新建 `<站点名> 收藏相册`，已有则复用，避免重复相册。
...
- 影响范围：本地相册图片预览 / 瀑布流布局；相册图片数量较多、持续懒加载、按文件名称排序时更明显。
- 查证结果：
  ...
  - 相册详情页传参为 `groupBy={gallerySort === 'time' ? 'day' : 'none'}`。因此按时间排序会进入批次 + 时间分组路径；按文件名排序会进入 `groupBy="none"` 路径，完全绕过批次拆分。
...
- 处理建议：
  ...
  - 相册详情页应明确传入 `batchSize={200}` 或复用统一常量，避免未来默认值漂移。
  - 相册详情页排序字段 / 排序顺序变化时，统一执行“可见数量重置为 200 + 滚动到顶部”。
- 修复摘要：
  ...
  - 相册详情排序控件新增升序 / 降序切换按钮，并将详情图片排序顺序独立持久化为 `galleryDetailSortOrder`，不复用相册列表排序顺序。
...
- 验证方式：
  ...
  - 页面测试：相册详情按文件名排序后点击加载更多，第 1 批 200 张仍保留在原批次容器，第 2 批作为新容器追加。
  - 组件测试：`groupBy="none"` 的批次容器应有 40px 底部间距。
  - 组件测试：`sortOrder="asc" | "desc"` 应影响文件名排序方向。
  - 组件测试：时间排序在跨多个批次时仍保持全局升序 / 降序，而不是每个批次各自排序。
  - 页面测试：相册详情加载到 1000 张后切换排序字段或排序顺序，应回到顶部且 `ImageListWrapper` 只接收首批 200 张图片。
