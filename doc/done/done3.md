...
| `hashPasswordSHA1()` | - | ✅ | ✅ 被调用 | ✅ | **已完成** |
| `testConnection()` | - | ✅ | ✅ 被调用 | ✅ | **已完成** |
| `votePost()` | `/post/vote.json` | ✅ | ✅ 被调用 | ✅ | **已完成** |
| `favoritePost()` | `/post/vote.json?score=3` | ✅ | ✅ 被调用 | ✅ | **已完成** |
| `unfavoritePost()` | `/post/vote.json?score=0` | ✅ | ✅ 被调用 | ✅ | **已完成** |
| `getPopularRecent()` | `/post/popular_recent.json` | ✅ | ✅ 被调用 | ✅ | **已完成** |
| `getPopularByDay()` | `/post/popular_by_day.json` | ✅ | ✅ 被调用 | ✅ | **已完成** |
| `getComments()` | `/comment.json` | ✅ | ✅ 被调用 | ✅ | **已完成** |
| `getFavoriteUsers()` | `/favorite/list_users.json` | ✅ | ✅ 被调用 | ✅ | **已完成** |

**状态说明**：
- **已完成**：客户端方法 + IPC Handler 调用 + Preload API 暴露，前端可调用（所有 API 均已完成全链路对接）
...
- [x] 在 `BooruTagSearchPage.tsx` 搜索结果中显示收藏状态 ✅
- [x] 创建 `FavoriteTagsPage.tsx` 收藏标签管理页面（列表、快速搜索、添加、编辑、删除）✅
- [x] 在收藏标签页面显示快速搜索标签云 ✅
- [x] 支持标签分组筛选（按站点筛选）✅
- [x] 支持拖拽排序（使用 `@dnd-kit/core`）✅
...
**UI 实现清单**：
- [x] 在 FavoriteTagsPage 工具栏添加导出/导入按钮（替代在设置页添加标签管理区域）✅
- [x] 添加"导出收藏标签"按钮（支持 JSON/TXT）✅
- [x] 添加"导出黑名单"按钮（支持 JSON/TXT）✅
- [x] 添加"导入标签"按钮 ✅
- [x] 显示导入结果统计（新增/跳过/错误）✅
- [x] 支持拖拽文件导入 ✅
- [x] 支持从剪贴板粘贴导入 ✅
...
#### 2.1 Yande.re/Moebooru 登录配置 [已完成]
...
**UI 实现清单**：
- [x] 在 `BooruSettingsPage.tsx` 添加登录表单（Modal 弹窗形式）✅
  - [x] 用户名输入框 ✅
  - [x] 密码输入框（输入后自动哈希，不存储明文）✅
  - [x] "登录"按钮 ✅
  - [x] 登录状态显示（已登录/未登录）✅
- [x] 添加站点 Salt 配置（高级选项，可手动输入）✅
- [x] 登录成功后刷新页面状态 ✅
...
**实现步骤**：
1. [x] 在 `channels.ts` 添加 IPC 通道定义 ✅
2. [x] 在 `handlers.ts` 添加 IPC 处理器 ✅
3. [x] 在 `preload/index.ts` 暴露 API ✅
4. [x] 实现 UI 组件 ✅

**UI 实现清单**：
- [x] 在 `BooruImageCard.tsx` 添加喜欢按钮（仅登录后显示）✅
- [x] 在 `Toolbar.tsx` (详情页) 添加投票按钮组 ✅
  - [x] 👍 喜欢按钮（score=3）✅
  - [x] 👎 不喜欢按钮（score=1）✅
  - [x] 取消投票选项 ✅
- [x] 在详情页显示"谁收藏了这张图片"✅
...
**UI 实现清单**：
- [x] 创建 `BooruServerFavoritesPage.tsx` 独立页面显示服务器收藏 ✅
- [x] 显示当前登录用户的服务器收藏 ✅
- [x] 添加"同步到本地"按钮 ✅
- [x] 添加"上传到服务器"按钮 ✅
- [x] 显示同步状态和进度 ✅
- [x] 处理同步冲突（本地有/服务器没有 等情况）✅
...
**UI 实现清单**：
- [x] 在 `BooruPostDetailsPage.tsx` 添加评论区 ✅
...
**UI 实现清单**：
- [x] 创建 `BooruPoolsPage.tsx` Pool 列表页面（含 Pool 详情视图）✅
- [x] 支持 Pool 搜索 ✅
- [x] Pool 内图片按顺序浏览 ✅
- [x] 在 App.tsx 添加路由 ✅
- [x] 在导航菜单添加"图集"入口 ✅
...
**已完成**：
- [x] i18n 框架（LocaleContext + useLocale hook）
- [x] 中文/英文语言文件（zh-CN.ts / en-US.ts）
- [x] 语言切换功能（设置页面 Segmented 组件）
- [x] App.tsx 菜单/标题/提示全面国际化
- [x] SettingsPage 全面国际化
- [x] Toolbar 组件国际化
- [x] Ant Design locale 联动切换
...
**已完成**：
- [x] useKeyboardShortcuts hook（支持修饰键组合、输入框屏蔽）
- [x] 快捷键帮助弹窗（Shift+? 打开）
- [x] 全局快捷键：主题切换(Ctrl+Shift+T)、设置(Ctrl+,)、搜索(Ctrl+F)
- [x] 导航快捷键：上一张/下一张(←→)、返回(Esc)、翻页(Alt+←→)
- [x] 操作快捷键：收藏(F)、下载(D)、打开原图(O)
...
|---|------|---------|
| 1 | 标签收藏功能 | 2026-03-06 |
| 2 | 标签黑名单功能 | 2026-03-06 |
| 4 | Yande.re 登录配置 | 2026-03-07 |
| 5 | 喜欢功能（Vote） | 2026-03-07 |
| 6 | 服务器收藏同步 | 2026-03-07 |
| 7 | 热门图片浏览 | 2026-03-07 |
| 8 | 评论功能 | 2026-03-07 |
| 9 | Pool（图集）浏览 | 2026-03-07 |
| 10 | 主题切换 | 2026-03-07 |
| - | 批量下载修复 + 按钮交互优化 | 2026-03-08 |
| 3 | 收藏/黑名单导入导出 | 2026-03-08 |
| - | UI 细节改进（收藏状态、用户列表、登录提示） | 2026-03-08 |
...
| - | 标签选择性导入（勾选）| ✅ 已完成 | 低 |
| - | 站点 Salt 高级配置 | ✅ 已完成 | 低 |
| - | 收藏同步到本地/上传到服务器 | ✅ 已完成 | 低 |
| - | 评论内容格式化显示 | ✅ 已完成 | 低 |
| - | Pool 上一张/下一张导航 | ✅ 已完成 | 低 |
...
> 所有功能已全部完成，包括 7 项低优先级增强任务。

---

### 已完成

> 详细记录见 `doc/done/done2.md`

- [x] **1.1 标签收藏功能** - 数据库表 + 服务层 + IPC + Preload + UI (2026-03-06)
- [x] **1.2 标签黑名单功能** - 数据库表 + 服务层 + 过滤逻辑 + UI (2026-03-06)
- [x] **2.1 登录配置** - 登录弹窗 + 密码哈希 + 状态存储 (2026-03-07)
- [x] **2.2 喜欢功能（Vote）** - IPC + Preload + UI 按钮 + 动画反馈 (2026-03-07)
- [x] **3.1 服务器收藏同步** - BooruServerFavoritesPage (2026-03-07)
- [x] **3.2 热门图片浏览** - BooruPopularPage + 日期选择 (2026-03-07)
- [x] **3.3 评论功能** - CommentSection + getComments/createComment (2026-03-07)
- [x] **3.4 Pool 浏览** - BooruPoolsPage + Pool 搜索 (2026-03-07)
- [x] **4.1 主题切换** - useTheme Hook + 明暗模式 + 设计令牌 (2026-03-07)
- [x] **批量下载自动恢复** - resumeRunningSessions (2026-03-06)
- [x] **普通下载功能强化** - 失败清除 + 重试 (2026-03-06)
- [x] **按钮与交互优化** - 13 项 P0-P2 任务全部完成 (2026-03-08)
- [x] **1.3 收藏/黑名单标签导入导出** - JSON/TXT 格式支持 + 黑名单导入导出 + UI 按钮 (2026-03-08)
- [x] **UI 细节改进** - 标签搜索页收藏状态、收藏用户列表、未登录提示 (2026-03-08)
- [x] **4.2 多语言支持** - i18n 框架 + 中英文语言文件 + 设置页语言切换 + 全面国际化 (2026-03-08)
- [x] **4.3 快捷键支持** - useKeyboardShortcuts hook + 快捷键帮助弹窗 + 全局/导航/操作快捷键 (2026-03-08)
- [x] **收藏标签拖拽排序** - @dnd-kit/core + sortable + 乐观更新 + sortOrder 持久化 (2026-03-08)

### 低优先级任务（已全部完成 2026-03-09）

- [x] 标签拖拽文件导入（拖拽 JSON/TXT 文件到页面自动导入）
- [x] 标签剪贴板粘贴导入（Ctrl+V 粘贴标签列表）
- [x] 标签选择性导入（导入前预览，勾选要导入的标签）
- [x] 站点 Salt 高级配置（在 BooruSettingsPage 添加手动输入 Salt 的选项）
- [x] 收藏双向同步（"同步到本地"/"上传到服务器"按钮 + 进度条）
- [x] 评论内容格式化显示（支持 DText/BBCode：加粗/斜体/删除线/引用/剧透/链接）
- [x] Pool 上一张/下一张导航（Pool 详情页内按顺序浏览 + 翻页导航栏）
