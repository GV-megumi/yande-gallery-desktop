...
- [x] 所有 queryType（tag / raw / list）均支持一键下载
- [x] `siteId == null` 的 favorite tag 禁止直接启动下载，需先有明确站点
...
- [x] 失败原因摘要与重试入口联动（历史弹窗中显示错误详情和重试按钮）
...
- [x] 支持绑定图集时自动创建图集
- [x] 下载完成后自动刷新 / 扫描对应图集
- [x] 支持图集绑定一致性修复提示（路径不一致时显示修复按钮和详情）
- [x] 支持按图集查看"来源 favorite tag"（图集信息弹窗中显示来源标签及下载状态）

---

## 收口与增强（已完成）

- [x] 为 favorite tag 下载流补更强的 DB/service/IPC 契约测试
- [x] 评估并优化 `getFavoriteTagsWithDownloadState()` 在大量标签下的聚合查询成本（批量查询替代 N+1）
- [x] 为 `FavoriteTagDownloadDisplayStatus` 建立更集中统一的状态映射工具（`src/shared/favoriteTagStatus.ts`）
...
- [x] `queryType === 'raw' | 'list'` 的下载语义（raw: 原始查询字符串直传; list: 空格分隔多标签拆分）
- [x] 历史统计面板（历史弹窗中显示总计/成功/失败统计摘要）
- [x] 基于图集维度的反向追踪视图（图集信息弹窗中显示来源 favorite tag 及下载状态）
