...
git commit -m "feat(api): agent 面权限映射表收缩——sync 与相册/图片写路由规则移除，回归设计文档 11 键"
```
...
- 若断言文本包含「图片写操作」「相册写操作」标签（SettingsPage.test.tsx），删除对应断言或改为断言标签不存在。
...
1. `API_PERMISSION_LABELS`（21-35 行）删除 `imageWrite: '图片写操作',` 与 `galleryWrite: '相册写操作',` 两行（类型 `Record<ApiServicePermissionKey, string>` 在 Task 1 收缩后会强制此改动）。
