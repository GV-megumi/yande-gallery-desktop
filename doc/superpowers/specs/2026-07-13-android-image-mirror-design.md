...
- 文件名 = 原图主文件名 + 按响应 Content-Type 定的实际扩展名；对安卓非法字符（`\ / : * ? " < > |`）清洗为 `_`，实际落盘名记录在 DB。
...
- **每轮算法**（增量天然断点续传——每次重算缺失集合）：
  ...
  4. 单图 404 → 计数跳过（元数据对账会删掉该图行，下轮不再出现）；**连续前 5 张全部 404 且 `service/info` 可达 → 中止并置同步错误「桌面端版本过旧，不支持高质量图档」**（旧桌面无 `/hq` 的兼容保护）。
