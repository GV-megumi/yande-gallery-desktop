...
| D4 | 同步触发 | 连接后自动增量（挂现有元数据同步总线），默认仅 WiFi，可设置允许移动网络；**增删都跟随**——桌面删图后手机镜像文件同步删除（见 §3.4 对账清理） |
...
- **对账清理**（镜像语义 = 增删都跟随桌面库，删除与下载同为同步的一部分）：
  - `RoomMirrorStore.clearMirror`（切服/dataVersion 变化）追加：清 `image_files` + 后台删除 `mirror/s{serverId}/` 目录。
  - 元数据 image-ids 对账删除图片行后：删除 `image_files` 中 imageId 不在 `images` 的行及其目录。**ORIGINAL 档同样跟随删除**——镜像不是本机收藏夹，用户手动下载的原图在桌面删图后也会清掉；长期留存诉求由未来「导出到相册」能力承载（D8/§9）。
