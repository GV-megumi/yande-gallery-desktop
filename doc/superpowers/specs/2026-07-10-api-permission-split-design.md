...
| 端点 | 来源 |
| --- | --- |
| `GET service/info`、`GET service/health` | 与 agent 面共享（连接测试/配对验证） |
...
1. 旧配置（input 与 current 均）无 `app.enabled` 时，初值取 `旧 permissions.imageWrite || 旧 permissions.galleryWrite`——写权限开着是「手机在用」的最强信号；只读手机用户升级后需手动重开一次，CHANGELOG 注明。
