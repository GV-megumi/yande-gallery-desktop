...
/**
 * 回归守卫：resumeRunningSessions 把同 taskId 双活跃坏数据自愈为
 * A=queued / B=paused 之后，scheduler 会把 A 从 queued 推回 pending 并再次
 * 调 startBulkDownloadSession(A)。此刻 A 走闸门时，如果把 'paused' 也算进
 * 活跃集，就会撞上 B（刚被 resume 降级为 paused 的"让路"伙伴），闸门直接
 * 返回 conflict，结果 A 也被挡回，用户看到的就是"恢复成功 1 条但卡在 pending"。
 *
 * 修复口径：startBulkDownloadSession 的看门调用必须 ignorePausedWhenProbing:true。
 * 这个测试断言：真实发给 DB 的活跃探测 SQL 里 **不包含 'paused'**。
 *
 * 实现取舍：不试图跑通整个 performDryRun 链路——那需要 mock getBooruSiteById /
 * createBooruClient / performDryRun 等一整套外部依赖。这里只在 "看门后 → 进入
 * dryRun 写入阶段" 这段路径上断言，让 booruClient.getPosts 在 performDryRun 里
 * 抛错提前终止测试流，然后从 getMock 的调用记录里找那条真正打到 DB 的探测 SQL。
 * 关键断言只聚焦在 "SQL 里不含 paused 且含 running" 这一点，够用且稳健。
 */
describe('bulkDownloadService.startBulkDownloadSession 看门忽略 paused 伙伴（反模式守卫）', () => {
  beforeEach(async () => {
    getMock.mockReset();
    runMock.mockReset();
    allMock.mockReset();
  ...
  afterEach(() => {
    vi.doUnmock('fs');
    vi.doUnmock('../../../src/main/services/booruClientFactory.js');
  });

  it('startSession 的闸门探测 SQL 必须从活跃集剔除 paused（否则 resume 后 keeper 会被 paused 伙伴挡住）', async () => {
    const sessionRow: any = {
      id: 'session-self',
      taskId: 'task-1',
      siteId: 1,
    ...
    // 核心探测器：
    // - 4-status 口径（无 paused）→ 无冲突，让闸门放行
    // - 5-status 口径（含 paused）→ 返回 {id:'session-paused-peer'}，如果真被这条命中
    //   将触发 conflict，从而暴露回归
    //
    // 这样即便实现细节（空格 / 引号 / 字段顺序）变化，只要 status 集合
    // 含 paused 就会被这个 mock 捕获成冲突。
    getMock.mockImplementation(async (_db: any, sql: string, _params?: any[]) => {
      if (sql.includes('FROM bulk_download_sessions s') && sql.includes('INNER JOIN bulk_download_tasks t')) {
        return sessionRow;
      }
      // 闸门活跃探测
      if (
        sql.includes('FROM bulk_download_sessions') &&
        sql.includes('WHERE taskId = ?') &&
        sql.includes('status IN')
      ) {
        if (sql.includes("'paused'")) {
          // 5-status 口径：故意返回一条 paused 伙伴，让 conflict 分支被命中
          return { id: 'session-paused-peer' };
        }
        // 4-status 口径：无冲突
        return undefined;
      }
      // countActiveSessions → 让闸门有空槽
      if (sql.includes("status IN ('dryRun', 'running')")) {
        return { n: 0 };
      }
      return null;
    ...
    // 让 performDryRun 快速失败，避免测试流进入长长的下载 IO；
    // 但此时闸门已经过完了——我们只需要 getMock 里捕获到那次探测调用。
    vi.doMock('../../../src/main/services/booruClientFactory.js', () => ({
      createBooruClient: vi.fn(() => ({
        getPosts: vi.fn(async () => { throw new Error('mock get posts fail'); }),
      })),
    ...
    const result = await startBulkDownloadSession('session-self');

    // 找到闸门真正发出去的那条 SQL
    const probeCall = getMock.mock.calls.find(args =>
      typeof args[1] === 'string' &&
      args[1].includes('FROM bulk_download_sessions') &&
      args[1].includes('WHERE taskId = ?') &&
      args[1].includes('status IN') &&
      !args[1].includes('INNER JOIN')
    );

    expect(probeCall).toBeDefined();
    const probeSql = String(probeCall![1]);

    // 关键守卫 1：活跃探测集合必须是 4 元组（不含 paused）
    expect(probeSql).toMatch(/status IN \('pending', 'queued', 'dryRun', 'running'\)/);
    // 关键守卫 2：不得退化回 5 元组口径
    expect(probeSql).not.toMatch(/'paused'/);

    // 关键守卫 3：闸门没有把本次 start 判成 conflict。
    // 实测 performDryRun 会让 result.success=false，但 error 一定不是
    // "该任务已有进行中的下载会话"。
    expect(result.error).not.toBe('该任务已有进行中的下载会话');
  });
});
