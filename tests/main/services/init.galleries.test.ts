...
describe('initGalleriesFromConfig 增量迁移 + 剥离旧配置 + 装载登记表', () => {
  it('增量迁移：DB 已有的图库跳过，仅迁入缺失的图库（DB 非空也迁移）', async () => {
    ...
    // Phase 8A：getAllGalleryFolderPaths 既是「存在则跳过」的去重源、又是登记表装载源（init 只查一次，
    // 迁移期间把新建路径累加进内存集合）。用有状态 mock 模拟「建库后该路径已绑定」：
    // 初始已绑定 M:/existing；createGallery 把新建路径推入数组，使去重/登记表呈现真实有状态行为。
    const boundFolders: string[] = ['M:/existing'];
    const createGallery = vi.fn(async (dto: { folderPath: string }) => {
      boundFolders.push(dto.folderPath);
      return { success: true, data: 10 };
    });
    const getGalleries = vi.fn(async () => ({ success: true, data: [{ id: 9 }] }));
    const getAllGalleryFolderPaths = vi.fn(async () => [...boundFolders]);
  ...
  it('DB 为空时全量迁移旧 config.folders 并装载登记表', async () => {
    ...
    const saveConfig = vi.fn(async () => ({ success: true }));
    vi.doMock('../../../src/main/services/config.js', () => ({ getConfig: vi.fn(() => cfg), saveConfig }));
    // DB 起始为空：有状态 mock，createGallery 把新建路径推入，登记表装载呈现「本次新建」
    const boundFolders: string[] = [];
    const createGallery = vi.fn(async (dto: { folderPath: string }) => {
      boundFolders.push(dto.folderPath);
