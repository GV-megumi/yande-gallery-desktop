...
// 修复轮 U11：apply 成功后要发 gallery:paths-relocated 领域事件，mock 掉发布器以便断言
...
describe('applyRelocateRoot — 领域事件 gallery:paths-relocated（修复轮 U11）', () => {
  // 背景：重定位只改路径字符串、不动 updatedAt，常驻缓存的图库页（导航缓存挂起态）
  // 靠既有增量事件（updatedAt 游标）感知不到任何变化，看起来"重定位没生效"。
  // apply 成功提交后必须发一条全量失效事件，让渲染层整体重载。

  it('成功改写 → 发一次事件，payload 含各站点 affected 与合计 totalCount，且发出时 app:// 白名单已刷新', async () => {
    const emitMock = emitGalleryPathsRelocated as unknown as Mock;
    let rootsAtEmit: string[] | null = null;
    // 订阅方收到事件会立即按新路径重载图片，事件必须发生在白名单刷新之后
    emitMock.mockImplementationOnce(() => {
      rootsAtEmit = getGalleryRootsSnapshot();
    });
    ...
    const payload = emitMock.mock.calls[0][0] as {
      affected: Array<{ table: string; column: string; count: number }>;
      totalCount: number;
    };
  ...
  it('UNIQUE 冲突中止（零写入）→ 不发事件', async () => {
    ...
    await addImage(normalizePath(path.join('N:', 'dst', 'dup.jpg'))); // 既有占位 → 冲突
