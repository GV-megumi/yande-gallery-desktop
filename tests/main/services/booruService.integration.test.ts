...
vi.mock('../../../src/main/services/database', () => ({
  ...
  all: vi.fn(async (_db, sql: string, params?: any[]) => {
    ...
    // Phase 4：getFavoriteTagsWithDownloadState 预取各相册的 gallery_folders 绑定文件夹
...
vi.mock('../../../src/main/services/galleryService', () => ({
  ...
  // Phase 4：下载路径校验改为"在相册 gallery_folders 内"；按 galleryId 返回其绑定文件夹
...
describe('booruService integration-ish behavior', () => {
  ...
  it('getFavoriteTagsWithDownloadState 应返回带 binding 和 galleryName 的 enriched 结果', async () => {
    ...
    // Phase 4：下载目录 D:/gallery/a 落在相册 gallery_folders 内 → 一致
  ...
  it('startFavoriteTagBulkDownload 下载目录不在相册 gallery_folders 内时应拒绝', async () => {
    // Phase 4：让该相册的绑定文件夹不含 binding.downloadPath('D:/gallery/a') → 校验失败抛错
    ...
    const service = await import('../../../src/main/services/booruService');
    await expect(service.startFavoriteTagBulkDownload(1)).rejects.toThrow('下载目录必须与绑定相册的文件夹路径一致');
  });
});
