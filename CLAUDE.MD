...
- `services/`：数据库、配置、图库扫描、缩略图、下载、缓存、Booru 客户端、更新检查与桌面通知等服务
- `ipc/`：统一 IPC 通道与处理器
- `index.ts`：主进程启动编排
...
- 当前主要分域：`db`、`gallery`、`config`、`booruPreferences`、`pagePreferences`、`image`、`booru`、`bulkDownload`、`window`、`system`

### 共享层 `src/shared/`

- 保存图库、Booru、下载、标签、配置事件等共享类型定义
...
### 7. Renderer API 以 preload 暴露面为准

`window.electronAPI` 的实际暴露面以 `src/preload/index.ts` 和 `src/preload/shared/*.ts` 为 source of truth。做渲染层能力、IPC 对接或页面调用排查时，优先看 preload，而不是只看历史说明。
...
- **改渲染层调用、预加载接口、IPC 对接**：`doc/Renderer API 文档.md`、`src/preload/index.ts`、`src/preload/shared/`、`src/main/ipc/channels.ts`
- **改下载、批量下载、收藏标签下载**：`doc/注意事项/下载与批量会话状态机.md`、`doc/Booru功能实现文档.md`
- **改导航缓存、固定页、页面偏好或子窗口**：`doc/注意事项/导航缓存与页面偏好持久化.md`、`doc/Renderer API 文档.md`
- **改桌面通知、托盘、外链、webview 或 Electron 安全边界**：`doc/注意事项/Electron桌面行为与通知.md`、`doc/注意事项/Electron安全边界.md`
...
- **发布新版本 / 打安装包**：`doc/版本发布打包规范.md`
- **看设计原因、坑点和约束**：`doc/注意事项/README.md`
- **追溯历史实现或旧方案**：`doc/done/README.md`

## 文档导航

- **当前主文档**：`doc/架构总览.md`、`doc/功能总览.md`、`doc/开发与配置指南.md`、`doc/数据库结构文档.md`、`doc/Renderer API 文档.md`、`doc/Booru功能实现文档.md`、`doc/图库功能文档.md`、`doc/版本发布打包规范.md`
